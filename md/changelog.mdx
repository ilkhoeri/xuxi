...
- Extended `cvx` so variant values can return strings, string arrays, or objects. The returned function now preserves native object and array values while retaining class-name composition for string-only variants.
