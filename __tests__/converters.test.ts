...
import * as xuxi from '../src/index'; // Namespace import
...
describe('export validation', () => {
  ...
  test('should include functions in namespace export', () => {
    expect(xuxi.rem).toBe(rem);
    expect(xuxi.em).toBe(em);
    expect(xuxi.px).toBe(px);
    expect(xuxi.createConverter).toBe(createConverter);
  });
});
