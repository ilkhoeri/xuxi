...
/**
 * @type {cvxVariants<T>} - Extracts the properties of the first argument of a given function type `T`, excluding `ExcludeKeys`.
 * @example
...
function cvx<T extends cvxKeys>(keys: cvxRecord<T>): (variants?: cvxResult<T>) => string {
  return (variants: cvxResult<T> = {}) => {
    const merg = { ...keys.defaultVariants, ...variants } as {
